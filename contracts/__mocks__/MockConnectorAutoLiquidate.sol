...
import { sub, wdiv, add } from "../math/DSMath.sol";
...
contract MockConnectorAutoLiquidate is ConnectorInterface {
    ...
    function getDataAndCastAutoLiquidation(
        uint256 _route,
        uint256 _vaultId,
        address _colToken
    ) external payable {
        (address[] memory targets, bytes[] memory datas) = _dataAutoLiquidation(_route, _vaultId, _colToken);
    ...
    // Call `flashBorrowAndCast` with or recipe to self liquidate the cdp
    function _dataAutoLiquidation(
        uint256 _route,
        uint256 _vaultId,
        address _colToken
        ...
        uint256 gasCost = _getGasConsumedAutoLiquidation(_route, _colToken);
        uint256 gasFeesPaidFromCol = _getGelatoProviderFees(gasCost, _colToken);

        (address[] memory _targets, bytes[] memory _datas) =
            _spellsAutoLiquidate(_vaultId, _colToken, wDaiToBorrow, gasFeesPaidFromCol);
    ...
    function _spellsAutoLiquidate(
        ...
        datas[3] = _encodePayGelatoProvider(_colToken, _gasFeesPaidFromCol, 0, 0);
