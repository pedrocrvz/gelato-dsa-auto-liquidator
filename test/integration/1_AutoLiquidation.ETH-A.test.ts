...
describe('[ETH-A] Auto Liquidation scenario where collateralization ratio goes lower than target set by user', function () {
  this.timeout(0)
  ...
  it('#3: Auto-Liquidate Vault, after the Maker vault became unsafe.', async function () {
    ...
    const gasCost = await getAutoLiquidationGasCostByRoute(route!, 'ETH-A')
